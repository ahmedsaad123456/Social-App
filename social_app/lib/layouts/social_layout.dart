  ...
  Widget build(BuildContext context) {
    return BlocConsumer<SocialCubit, SocialStates>(
      ...
      builder: (context, state) {
        ...
        return Scaffold( 
          appBar: AppBar(
            title: Text(cubit.titles[cubit.currentIndex]), // Display current screen title
            actions: [
              IconButton(
                icon: const Icon(
          ...
          bottomNavigationBar: BottomNavigationBar(
            currentIndex: cubit.currentIndex, // Current index for bottom navigation
            onTap: (value) {
              cubit.changeBottomNavBar(value); // Change the current screen index
            },
            items: const [ // Bottom navigation items
              BottomNavigationBarItem(
                  icon: Icon(IconBroken.Home), label: 'Home'), // Home screen
              BottomNavigationBarItem(
                  icon: Icon(IconBroken.Chat), label: 'Chats'), // Chat screen
              BottomNavigationBarItem(
                  icon: Icon(IconBroken.Paper_Upload), label: 'Post'), // Post screen
              BottomNavigationBarItem(
                  icon: Icon(IconBroken.Location), label: 'Users'), // Users screen
              BottomNavigationBarItem(
                  icon: Icon(IconBroken.Setting), label: 'Settings'), // Settings screen
            ],
          ),
        );
      },
